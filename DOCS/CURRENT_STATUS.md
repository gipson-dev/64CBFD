...
denominator driven: the exact count is now 2,649, while
...
3. The original `func_150AD780`/`func_150AD78C` trig slice is restored, and
   ...
   Continue with 23-word `func_15167010`, the first ordinary-C row in the
   fresh 15-difference queue; it traverses the fixed `struct115` table and
   invokes each non-null callback at offset `0x18`.
