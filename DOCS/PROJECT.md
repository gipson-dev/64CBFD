...
| --- | ---: | ---: |
| Total | `[#####################---]` 85.64% | 5,484 / 6,038 (90.82%) |
| Init | `[######################--]` 90.79% | 508 / 538 (94.42%) |
| Game | `[####################----]` 85.11% | 4,795 / 5,318 (90.17%) |
| Debugger | `[########################]` 99.19% | 181 / 182 (99.45%) |

There are 554 tracked raw-assembly functions. The increase from July reflects
...
| --- | ---: | ---: | ---: |
| Total | `[###########-------------]` 2,575 / 5,484 (46.95%) | 1 | 2,908 |
| Init | `[##################------]` 387 / 508 (76.18%) | 1 | 120 |
| Game | `[##########--------------]` 2,007 / 4,795 (41.86%) | 0 | 2,788 |
...
The exact count is forty-one above the July snapshot. The higher percentage is still
primarily caused by the smaller C denominator; focused event-swap and debugger
passes have added forty-three exact functions after the restoration baseline. See
...
helpers, generated-slice padding tools, and self-contained raw-object/minimal
ELF wrappers. Run `make tools-check` to validate the local MIPS wrappers
without requiring a ROM. See [Project tools](TOOLS.md) for commands,
toolchain overrides, and the compatibility status of imported generators.
