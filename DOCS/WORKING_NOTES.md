...
**Active (2026-09-26, row-list head insertion matched).** The current linked
checkpoint is `2667 / 5483 (48.64%)` exact C functions, with one address-drift
blocker and 2,815 genuinely different C functions. The tree
...
six-difference game tier is complete. `func_15019BB8` now matches through
...
through guarded outer/child pointer register lifetimes. Adjacent
`func_151D7770` and `func_151D779C` now match from source-level
...
`func_15074A94` then matched from a direct interpolation expression plus
...
from corrected byte and forwarded-result callee contracts. `func_151ACA20`
then matched directly from candidate-first local declaration and a single
...
pointer/value-register words. `func_150882B0` then matched from pointer-first
...
and pointer-correct callee declaration. `func_151355B8` then matched after
volatile field accesses restored the duplicate store and five guarded words
selected retail's loaded/result registers. `func_15168800` then matched
directly from an explicit early null return, with no guarded words.
`func_151E5FAC` then matched from duplicated fallback returns, positive
threshold control flow, and twelve guarded relocation/scheduling words,
closing the twelve-difference game tier. `func_15002560` then matched from an
...
source-field loads. Placeholder `func_15131918` then became byte-exact C after
...
bytes. `func_1518F45C` then matched directly after its scalar array became a
...
restored retail's explicit pointer arithmetic. `func_1518F89C` then matched
from a typed float-record view plus twelve guarded scheduling words and one
inserted call-delay `nop`. `func_151A561C` then matched directly from the same
one-word aggregate and `void *` callee contract used by `func_1518F45C`.
`func_151D343C` then matched from the third one-word aggregate and a corrected
...
the two omitted epilogue words. `func_15168A4C` then matched after recovering
typed node and explicit row/column scalar lifetimes, plus four guarded
`a2`-versus-`t0` register-color words. Keep generated unaligned-load helpers
`func_151F892C` and `func_151F8960` in the raw-assembly queue and continue
with 23-word `func_150747E4`. The latest result is in
...
The completed viewport setup frame/address normalization is in
